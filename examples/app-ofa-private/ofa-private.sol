...
contract OFAPrivate {
    ...
    function saveOrder() internal view returns (HintOrder memory) {
        ...
        Suave.DataRecord memory bid = Suave.newDataRecord(10, addressList, addressList, "");
    ...
    function newMatch(Suave.DataId shareDataId) external payable returns (bytes memory) {
        HintOrder memory hintOrder = saveOrder();
    ...
    function emitMatchBidAndHint(string memory builderUrl, Suave.DataId bidId) external payable returns (bytes memory) {
